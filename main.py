import requests
import time, logging
from bloxflip import *

token = ""


logging.basicConfig(filename='logs.log', encoding='utf-8', level=logging.INFO ,format="%(asctime)s: %(message)s", datefmt="%m/%d/%Y %I:%M:%S %p")

previous_block = None
betColor = "red"

betAmount = 2
interruptor = 0


class main:
    def __init__(self) -> None:
        self.slideBet()

    def slideBet(self):
        global previous_block
        global betAmount
        global betColor
        global interruptor
        
        while True:
            time.sleep(3)
            response = requests.get('https://projectmambas.shyzos.xyz/api/games/roulette')
            data = response.json()

            if not data['success']:
                print("Erreur lors de la récupération des résultats.")
                logging.info("Error: API")
                continue

            current_block = data['eosBlock']
            color = data['winColor']

            if previous_block is None or previous_block != current_block:
                if color == betColor:

                    betAmount = 1

                    if interruptor == 0:
                        pass

                    else:
                        interruptor -= 1

                else:
                    if previous_block == None:
                        betAmount = 1
                    
                    else:
                        betAmount *= 2

                    if interruptor == 3:
                        if betColor == "red":
                            betColor = "purple"
                            logging.info("new target color: PURPLE")

                        else:
                            betColor = "red"
                            logging.info("new target color: RED")

                    elif interruptor > 3:
                        interruptor = 3

                    else:
                        interruptor += 1
                    

                logging.info(f"colorWin: {color}")
                logging.info(f"betAmount: {betAmount}")

                print(f"\n\ncolorWin: {color}")
                print(f"betAmount: {betAmount}")


                time.sleep(15)
                async def main():

                    await slide(token).bet(betColor, betAmount)

                asyncio.run(main())
                

                previous_block = current_block
            
            

# Exécution du script
main()
